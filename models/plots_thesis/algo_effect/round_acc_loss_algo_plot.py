...
def plot_combined_accuracy_loss(df, baseline_data, metrics):
    ...
    for metric, color in zip(metrics, colors_accuracy):
        ax_accuracy.plot(df.index, df[f'CFR_300r_1e_{metric} - round_accuracy'] * 100, label=f'{metric} Accuracy', color=color)

    # Plot Baseline Accuracy
    baseline_accuracy = [epoch['val_accuracy'] for epoch in baseline_data['epochs']][:300]
    ...
    fig_accuracy.savefig('plots/algo_effect_combined_accuracy_plot.pdf', format='pdf')
    ...
    for metric, color in zip(metrics, colors_loss):
        ax_loss.plot(df.index, df[f'CFR_300r_1e_{metric} - round_loss'], label=f'{metric} Loss', color=color)

    # Plot Baseline Loss
    baseline_loss = [epoch['train_loss'] for epoch in baseline_data['epochs']][:300]
    ...
    fig_loss.savefig('plots/algo_effect_combined_loss_plot.pdf', format='pdf')
    plt.close(fig_loss)

# Load the data
csv_path = 'updated_cfr_300r1e_loss_acc.csv'
