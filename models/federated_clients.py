...
class GradientBasedFedAvgClient(FedClient):
    ...
    def train_local_model(self, model, global_round, client_idx):
        ...
        # Set optimizer for the local updates
        if self.args.optimizer == 'sgd':
            self.optimizer = torch.optim.SGD(model.parameters(), lr=self.args.lr, momentum=0.5)
        elif self.args.optimizer == 'adam':
            self.optimizer = torch.optim.Adam(model.parameters(), lr=self.args.lr, weight_decay=1e-4)
    ...
    def train_clients(self, global_model, round_, global_model_non_encry=None):
        ...
        global_model_weights = self.update_model_weights_with_aggregated_grads(aggregated_grads, global_model)
        if self.args.train_without_encryption:
            aggregated_grads_nonencr = self.server.average_weights_nonencrypted(local_grads)
            global_model_weights_nonencr = self.update_model_weights_with_aggregated_grads(aggregated_grads_nonencr,
